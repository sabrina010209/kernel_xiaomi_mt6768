...
extern int nfc_probe(struct i2c_client *client, const struct i2c_device_id *id);
extern int nfc_remove(struct i2c_client *client);
                               ...
                               const struct i2c_device_id *id, chip_t chip_type)
{
    ...
    case NXP_PN557:
        ...
        TMS_INFO("NXP pn557 driver start probe\n");
        nfc_probe(client, id);
        break;
...
static int guidev_chip_match(struct guide_dev *gdata)
{
    ...
    for (count = 0; count < ARRAY_SIZE(info_list); count++) {
        ...
        } else if (info_list[count].type == NXP_PN557) {
...
static int guidev_dev_remove(struct i2c_client *client)
{
    ...
    case NXP_PN557:
        TMS_INFO("NXP PN557 nfc start removed\n");
        nfc_remove(client);
        break;
...
static int guide_suspend(struct device *device)
{
    ...
    case NXP_PN557:
        TMS_INFO("NXP PN557 nfc start suspend\n");
...
static int guide_resume(struct device *device)
{
    ...
    case NXP_PN557:
        TMS_INFO("NXP PN557 nfc start resume\n");

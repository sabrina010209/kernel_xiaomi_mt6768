...
static void silfp_auth_speed_up(struct work_struct *work);
...
static
#endif
fp_debug_level_t sil_debug_level = ERR_LOG;
...
static int silfp_init(struct silfp_data *fp_dev)
{
    ...
    INIT_WORK(&fp_dev->work2, silfp_auth_speed_up);
...
#define FP_UNLOCK_REJECTION_TIMEOUT (WAKELOCK_HOLD_TIME - 500) /* in ms */
extern int mdss_prim_panel_fb_unblank(int timeout);
static void silfp_auth_speed_up(struct work_struct *work)
{
    int ret = mdss_prim_panel_fb_unblank(FP_UNLOCK_REJECTION_TIMEOUT);
    LOG_MSG_DEBUG(INFO_LOG, "[%s] mdss_prim_panel_fb_unblank, ret = %d\n", __func__, ret);
}
...
static int silfp_set_feature(struct silfp_data *fp_dev, u8 feature)
{
    ...
    case FEATURE_AUTH_SPEED_UP:
        LOG_MSG_DEBUG(INFO_LOG, "[%s] auth speed up\n", __func__);
        schedule_work(&fp_dev->work2);
        break;

...
#define lct_testing_sysfs_raw_delta_show(t_name) \
static ssize_t lct_testing_sysfs_##t_name##_show(struct seq_file *file, char* buf) \
{ \
	int retval; \
	int count; \
	int rows, cols, i, j; \
...
lct_testing_sysfs_raw_delta_show(raw_data)
lct_testing_sysfs_raw_delta_show(delta_data)
EXPORT_SYMBOL(lct_testing_sysfs_raw_data_show);
EXPORT_SYMBOL(lct_testing_sysfs_delta_data_show);

static struct proc_dir_entry *lct_ovt_tp_data_dump_entry;
static char ovt_dump_data[4096] = {0};

int lct_ovt_tp_data_dump_proc_show(struct seq_file *file, void* data)
{
	//print raw data
	seq_printf(file,"\nRAW DATA\n");
    lct_testing_sysfs_raw_data_show(file, ovt_dump_data);

    memset(ovt_dump_data, 0, sizeof(ovt_dump_data));

    //print diff data
	seq_printf(file,"\nDIFF DATA\n");
    lct_testing_sysfs_delta_data_show(file, ovt_dump_data);

	return 0;
}

static int lct_ovt_tp_data_dump_open(struct inode* inode, struct file* file)
{
	return single_open(file, lct_ovt_tp_data_dump_proc_show, NULL);
}

static const struct file_operations lct_ovt_tp_data_dump_fops = {
	.owner = THIS_MODULE,
	.open = lct_ovt_tp_data_dump_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = seq_release,
};

int lct_ovt_tp_data_dump_proc_init(void)
{
	lct_ovt_tp_data_dump_entry = proc_create(LCT_TP_DATA_DUMP, 0444, NULL, &lct_ovt_tp_data_dump_fops);
	if (lct_ovt_tp_data_dump_entry == NULL) {
		printk("Enter %s(%d): create proc/%s Failed!\n", __func__, __LINE__, LCT_TP_DATA_DUMP);
		return -ENOMEM;
	} else {
		printk("Enter %s(%d): create proc/%s Succeeded!\n", __func__, __LINE__, LCT_TP_DATA_DUMP);
	}

    return 0;
}

void lct_ovt_tp_data_dump_proc_exit(void)
{
	if (lct_ovt_tp_data_dump_entry != NULL) {
		remove_proc_entry(LCT_TP_DATA_DUMP, NULL);
		lct_ovt_tp_data_dump_entry = NULL;
		printk("Enter %s(%d): Removed /proc/%s\n", __func__, __LINE__, LCT_TP_DATA_DUMP);
	}
}
#endif
...
static int testing_do_testing(void)
{
	...
	if (error_count) {
		//test fail result
		sprintf(file_path, "/data/vendor/touchpad/tp_%s_test_data_%02d%02d%02d-%02d%02d%02d-fail.csv", tcm_hcd->id_info.part_number,
            (rtc_now_time.tm_year + 1900) % 100, rtc_now_time.tm_mon + 1, rtc_now_time.tm_mday,
            rtc_now_time.tm_hour, rtc_now_time.tm_min, rtc_now_time.tm_sec);
	} else {
		//test pass result
		sprintf(file_path, "/data/vendor/touchpad/tp_%s_test_data_%02d%02d%02d-%02d%02d%02d-success.csv", tcm_hcd->id_info.part_number,
            (rtc_now_time.tm_year + 1900) % 100, rtc_now_time.tm_mon + 1, rtc_now_time.tm_mday,
            rtc_now_time.tm_hour, rtc_now_time.tm_min, rtc_now_time.tm_sec);
	}
...
int lct_ovt_tp_selftest(void)
{
    ...
    test_result = testing_do_testing();
    if (test_result < 0) {
		printk("lct %s(%d): tp selftest Failed\n", __func__, __LINE__);
        return 1;
    } else {
		printk("lct %s(%d): tp selftest Passed\n", __func__, __LINE__);
		return 2;
    }
}
...
int lct_ovt_tp_selftest_callback(unsigned char cmd)
{
	...
	case TP_SELFTEST_CMD_XIAOMI_SHORT:
		...
		break;
	default:
		printk("lct %s(%d): touch selftest command: unknow\n", __func__, __LINE__);
		break;
	}

...
{
    int ret = 0;
    if (!(strcmp(touch_lcm_name, "dsi_panel_c3u_35_03_0b_dsc_vdo"))) {
    return -1;
    }
    FTS_FUNC_ENTER();
...
static struct spi_driver fts_ts_spi_driver = {
 ...
 int __init lcm_info_detect(char *str)
{
	strlcpy(touch_lcm_name, str, sizeof(touch_lcm_name));
    printk("%s:%s\n",__func__, touch_lcm_name);
	return 0;
}
__setup("LCM_name=", lcm_info_detect);


#ifdef LCT_TP_INFO_EN
static int __init lct_tp_fts_is_lockdown_info_detect(char *str)
{
...
__setup("tp_lockdown_info=", lct_tp_fts_is_lockdown_info_detect);
#endif

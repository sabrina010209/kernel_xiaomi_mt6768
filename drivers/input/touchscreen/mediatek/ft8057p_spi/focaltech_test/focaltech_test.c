...
static int fts_tp_data_dump_proc_init(void)
{
	...
	lct_tp_data_dump_p->tp_data_dump_proc = proc_create(LCT_TP_DATA_DUMP, 0444, NULL, &tp_data_dump_proc_fops);
	if (IS_ERR_OR_NULL(lct_tp_data_dump_p->tp_data_dump_proc)) {
		FTS_TEST_ERROR( "ERROR: create /proc/%s failed.", LCT_TP_DATA_DUMP );
		lct_tp_data_dump_p->tp_data_dump_proc = NULL;
		ret = -1;
		goto err_create_procfs_fail;

	}
	FTS_INFO("create /proc/%s", LCT_TP_DATA_DUMP);
	return 0;
...
static void fts_tp_data_dump_proc_exit(void)
{
	...
	if (!IS_ERR_OR_NULL(lct_tp_data_dump_p->tp_data_dump_proc)) {
		...
		FTS_INFO("remove /proc/%s", LCT_TP_DATA_DUMP);
